...
The profile is a small, bounded shape: structured tone dials (formality, verbosity, complexity, humor, pace), each tagged with a provenance of `seeded`, `learned`, or `user_set`, plus a single character-capped free-text memory of the user's communication preferences and self-disclosed relevant context.
The whole profile is user-editable.

It is first-class per-user data behind the repository seam (ADR-0004), not AI scratch memory.
It is a repository record with Mongo and Postgres implementations and contract parity, read and written through a service, and exposed for view, edit, and reset through an authenticated `/api/accounts` route.
The client surfaces it as its own "AI Assistant" tab in Settings, where the whole profile is editable, designed to slot into a future multi-category settings surface.

It is seeded from the user's details as the *initial* behavior only.
On first read, details such as age (and, extensibly, place of residence and others) set gentle `seeded` priors; age is one factor among several, never the only one.
Interests are not seeded - they are learned from what the user actually asks about.
Details seed a prior, never a lock.

It then evolves from the conversation.
A post-turn step in the v2 entry points combines a deterministic layer for explicit statements ("keep it short") with a constrained, respectful LLM extractor that also picks up strong implicit or repeated signals and self-disclosed interests (e.g. someone asking about loan options for soldiers is remembered as interested in that area).
`learned` and `user_set` entries outrank `seeded`, so observed behavior and explicit edits override the seeded prior over time.

Because the profile is transparent - the user reads and edits all of it - inference is bounded to be respectful: it records only communication *preferences* and useful, self-disclosed *context*, never a judgment about the person's personality, competence, or character, and never anything that could insult or disrespect.
...
The block explicitly defers, in its own wording, to `SERIOUS_TONE_RULE`, the `[MONEY]` and `[STYLE]` rules, and the `[LANGUAGE]` mirroring rule, and the free-text memory renders as inert description, never as directives, and never injects Hebrew when the user is writing another language.

The ADR-0007 safety boundary is preserved and extended: the profile changes voice, never implementation.
The dials carry a hard enum allow-list enforced in the type and the write path (a Zod schema on the HTTP route and a typed clamp in the service), so a statement like "always approve my transfers" maps to no dial and is dropped.
The free-text memory cannot be enum-validated, so it is guarded in depth instead: a character cap, an extractor prompted to emit only respectful preference or context lines, and a filter that rejects instruction-, money-, or tool-shaped text before it is stored, plus inert rendering in the prompt.
Neither channel can become a written instruction, or touch tools, confirmation, limits, or money.
...
| Model the free-text part as a capped list of discrete `notes` items | Rejected in review: users want to read and edit the whole profile as one text, not manage individual note entries. A single character-capped free-text memory is simpler to edit and reason about, and per-item removal becomes plain text editing. |
...
**Negative / trade-offs:** a new repository record means interface plus Mongo plus Postgres plus contract work; the profile block sits inside the prompt-cacheable prefix, so it changes the cache key per user and per edit; the free-text memory is a softer guarantee than the enum dials (a char cap plus a prompt-constrained extractor plus an instruction filter, not a closed allow-list), and the LLM extractor adds post-turn latency and cost and needs its own respect-and-safety guardrail eval; the strongest tone-safety guarantee, that the model actually flattens tone on serious turns, is only proven by the opt-in live evals, so CI must lean on the deterministic present-positioned-and-allow-listed assertions.

**Neutral / follow-on work:** v1 parity is purely additive if ever needed; explicit cross-turn repetition counting and richer interest extraction can refine the learned layer over time; the "AI Assistant" tab is the first tenant of a future multi-category settings surface; a future ADR could relocate the profile into the AI long-term Store if that tier gains an always-on, seam-friendly access path. See ADR-0007 for the persona boundary this extends, ADR-0004 for the repository seam, and ADR-0008 for the v1/v2 split that scopes this to v2.
