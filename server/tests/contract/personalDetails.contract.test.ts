...
describeContract("PersonalDetailsRepository", {
  ...
  "ensureForUser creates a not_provided row with 24-hex id": async ({ repos }) => {
    ...
    assert.match(pd.id, /^[0-9a-fA-F]{24}$/);
    assert.equal(pd.userId, user.id);
    assert.equal(pd.status, "not_provided");
    assert.equal(pd.firstName, null);
    assert.equal(pd.lastName, null);
    assert.equal(pd.dateOfBirth, null);
    assert.deepEqual(pd.address, {});
    assert.equal(pd.lastSkippedAt, null);
    assert.ok(pd.createdAt instanceof Date);
    assert.ok(pd.updatedAt instanceof Date);
  },
  ...
  "ensureForUser is idempotent (second call returns same row, no duplicate)": async ({ repos }) => {
    ...
    assert.equal(second.status, "not_provided");
  ...
  "findByUserId returns the row after ensureForUser": async ({ repos }) => {
    ...
    assert.equal(found.status, "not_provided");
  },
  ...
  "update applies patch fields and returns updated record": async ({ repos }) => {
    ...
    assert.ok(updated);
    assert.equal(updated.status, "provided");
    assert.equal(updated.firstName, "Alice");
    assert.equal(updated.lastName, "Smith");
    assert.ok(updated.dateOfBirth instanceof Date);
    assert.equal(updated.dateOfBirth.toISOString(), dob.toISOString());
    assert.deepEqual(updated.address, { city: "Tel Aviv", country: "IL" });
  },
  ...
  "update only touches specified patch fields (partial update)": async ({ repos }) => {
    ...
    assert.equal(afterFirst?.status, "not_provided"); // untouched
    ...
    assert.equal(afterSecond?.status, "provided");
    assert.equal(afterSecond?.firstName, "Bob"); // still set
  },
  ...
  "findProvidedByUserIds returns only status=provided rows matching the id set": async ({ repos }) => {
    ...
    assert.deepEqual(ids, [u1.id, u2.id].sort());
    assert.ok(results.every((r) => r.status === "provided"));
  },
  ...
  "findProvidedByUserIds excludes user ids not in the set": async ({ repos }) => {
    ...
    assert.equal(results[0].userId, u1.id);
  },
  ...
  "address (jsonb object) round-trips through insert and update": async ({ repos }) => {
    ...
    const found = await repos.personalDetails.findByUserId(user.id);
    assert.ok(found);
    assert.deepEqual(found.address, addr);
  }
});
