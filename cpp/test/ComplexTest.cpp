...
static void TestMultiplyDivide(void)
{
    ...
    // Binary operator/: (1+2i) / (3+4i) = (1*3 + 2*4)/(3^2+4^2) + (2*3 - 1*4)/(3^2+4^2)i
    // = (3+8)/25 + (6-4)/25*i = 11/25 + 2/25*i = 0.44 + 0.08i
    Complex c4 = c1 / c2;
    RUN_TEST(TestMultiplyDivide, "c4.GetReal() == 0.44f", 0.44f == c4.GetReal());
...
static void TestStreamOperator(void)
{
    ...
    RUN_TEST(TestStreamOperator, "oss1.str() is not empty", !oss1.str().empty());
    RUN_TEST(TestStreamOperator, "oss2.str() is not empty", !oss2.str().empty());
    RUN_TEST(TestStreamOperator, "oss3.str() is not empty", !oss3.str().empty());
    RUN_TEST(TestStreamOperator, "oss4.str() is not empty", !oss4.str().empty());
    ...
    RUN_TEST(TestStreamOperator, "oss1.str() == \"{3 + 4i}\"", 0 == strcmp("{3 + 4i}", oss1.str().c_str()));
    RUN_TEST(TestStreamOperator, "oss2.str() == \"{-1 - 2i}\"", 0 == strcmp("{-1 - 2i}", oss2.str().c_str()));
    RUN_TEST(TestStreamOperator, "oss3.str() == \"{0 + 5i}\"", 0 == strcmp("{0 + 5i}", oss3.str().c_str()));
    RUN_TEST(TestStreamOperator, "oss4.str() == \"{7 + 0i}\"", 0 == strcmp("{7 + 0i}", oss4.str().c_str()));
