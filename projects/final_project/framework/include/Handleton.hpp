...
/**
 * @file Handleton.hpp
 * @brief Declares the process-wide singleton helper used by legacy services.
 */
...
/**
 * @brief Lazily creates one process-wide instance of `T`.
 * @tparam T Type managed as a singleton.
 *
 * `T` must be default-constructible and accessible from this template.
 */
template <typename T>
class Handleton
{
  public:
     ...
     * @return Pointer to the single `T` instance.
     */
...
/**
 * @brief Legacy pre-C++11 Handleton implementation guarded by a custom mutex.
 * @tparam T Type managed as a singleton.
 */
template <typename T>
class Handleton
...
/**
 * @brief Explicitly instantiates Handleton for a concrete type.
 * @param T Concrete type to instantiate.
 */
...
/**
 * @brief Declaration-only view of the Handleton singleton helper.
 * @tparam T Type managed as a singleton.
 */
template <class T>
class Handleton
