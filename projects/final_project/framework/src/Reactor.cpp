...
void Reactor::AddFd(int fd, IListener::Mode mode, const ActionFunc& proc)
{
    ...
    ILRD_DEBUG_LOG("Reactor added fd=" + std::to_string(fd) + " mode=" +
...
void Reactor::RemoveFd(int fd, IListener::Mode mode)
{
    ...
    ILRD_DEBUG_LOG("Reactor removed fd=" + std::to_string(fd) + " mode=" +
                   std::to_string(static_cast<int>(mode)));
}
...
void Reactor::Run()
{
    ...
    m_isRunning = true;
    m_stopRequested = false;
    ILRD_DEBUG_LOG("Reactor loop started with " +
                   std::to_string(m_actions.size()) + " registered actions");
    ...
    try
    {
        while (!m_stopRequested && !m_actions.empty())
        {
            ...
            ILRD_DEBUG_LOG("Reactor received " +
                           std::to_string(ready.size()) + " ready descriptors");
            ...
            for (std::size_t i = 0; i < ready.size() && !m_stopRequested; ++i)
            {
                ...
                ILRD_DEBUG_LOG("Reactor invoking callback for fd=" +
                               std::to_string(ready[i].second) + " mode=" +
                               std::to_string(static_cast<int>(ready[i].first)));
    ...
    m_isRunning = false;
    ILRD_DEBUG_LOG("Reactor loop stopped");
}

void Reactor::Stop()
{
    m_stopRequested = true;
    ILRD_DEBUG_LOG("Reactor stop requested");
}
