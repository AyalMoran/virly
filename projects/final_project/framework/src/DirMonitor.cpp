...
struct DirMonitor::Impl
{
    explicit Impl(const std::string& path_name)
        : m_directory(std::filesystem::canonical(path_name).string()),
          ...
          m_thread()
    {
        ILRD_DEBUG_LOG("DirMonitor initializing for " + m_directory);
        if (m_inotifyFd == -1)
        {
        ...
        ILRD_DEBUG_LOG("DirMonitor thread started for " + m_directory);
    }

    ~Impl()
    {
        ILRD_DEBUG_LOG("DirMonitor shutting down for " + m_directory);
        if (m_stopPipe[1] != -1)
        {
    ...
    void SubscribeAdded(DirMonitor::Callback callback)
    {
        ...
        ILRD_DEBUG_LOG("DirMonitor subscribed add callback");
    }
    ...
    void SubscribeDeleted(DirMonitor::Callback callback)
    {
        ...
        ILRD_DEBUG_LOG("DirMonitor subscribed delete callback");
    }
    ...
    void SubscribeModified(DirMonitor::Callback callback)
    {
        ...
        ILRD_DEBUG_LOG("DirMonitor subscribed modify callback");
    }
    ...
    void MonitorLoop()
    {
        ...
        while (true)
        {
            ...
            {
                ILRD_DEBUG_LOG("DirMonitor received stop signal");
                return;
            }
    ...
    void HandleEvent(const inotify_event& event)
    {
        ...
        {
            ILRD_DEBUG_LOG("DirMonitor detected added path " + full_path);
            NotifyAdded(full_path);
        }

        if (0 != (event.mask & (IN_DELETE | IN_MOVED_FROM)))
        {
            ILRD_DEBUG_LOG("DirMonitor detected deleted path " + full_path);
            NotifyDeleted(full_path);
        }

        if (0 != (event.mask & (IN_MODIFY | IN_CLOSE_WRITE)))
        {
            ILRD_DEBUG_LOG("DirMonitor detected modified path " + full_path);
