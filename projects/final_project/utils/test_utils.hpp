...
#define ASSERT_EQ(suite, expected, actual)                                    \
    TestUtils::RunTest((suite), #expected " == " #actual,                     \
...
#define ASSERT_NULL(suite, ptr)                                               \
    TestUtils::RunTest((suite), #ptr " == NULL", (ptr) == nullptr, __LINE__,  \
                       __FILE__)
...
#define ASSERT_STR_EQ(suite, expected, actual)                                \
    TestUtils::RunTest((suite), #expected " == " #actual,                     \
                       std::strcmp((expected), (actual)) == 0, __LINE__,      \
                       __FILE__)

#define ASSERT_MEM_EQ(suite, ptr1, ptr2, size)                                \
    TestUtils::RunTest((suite), "memcmp(" #ptr1 ", " #ptr2 ", " #size         \
                                ") == 0",                                     \
                       std::memcmp(ptr1, ptr2, size) == 0, __LINE__, __FILE__)
