...
static knight_status_t RecKnightHelper(pos_xy_t pos, path_t res_path, bits_arr_t board, size_t curr_step);
...
knight_status_t KnightSolveBacktracking(pos_xy_t pos, path_t res_path)
{
    ...
    return RecKnightHelper(pos, res_path, board, 0);
}
...
static knight_status_t RecKnightHelper(pos_xy_t pos, path_t res_path, bits_arr_t board, size_t curr_step)
{
    ...
    {
        int next_row = row + drows[i];
        int next_col = col + dcols[i];
        ...
        if (TRUE == MoveIsSafe(next_row, next_col, board))
        {
            ...
            moves[move_cnt].degree =
                CountMoves(next_row, next_col, board);
            ++move_cnt;
        }
    }
    ...
    {
        new_board = BitsArrSetOn(board, (pos_xy_t)KnightTourGetRow(moves[i].pt) * BOARD_DIM + KnightTourGetColumn(moves[i].pt));
        res_path[curr_step + 1] = moves[i].pt;

        if (KNIGHT_SUCCESS == RecKnightHelper(moves[i].pt, res_path, new_board, curr_step + 1))
        {
